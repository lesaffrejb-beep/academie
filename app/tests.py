...
SUITES = [
    ...
    ("chronologie du journal", "tests_chronologie.py"),
    ...
    ("preparation migration v2", "tests_migration.py"),
    ("usine pas à pas", "tests_usine.py"),
    ("programme", "tests_programme.py"),
    ("programme IFSI 2026", "tests_ifsi.py"),
    ("corrections copro et artefacts", "tests_copro.py"),
...
MUTATIONS = [
    ("un score numérique certifie un geste clinique", "valide_ifsi.py",
     'if validation.get("certification_clinique") is not False:',
     'if False:'),
    ("l'import change le nonce d'un evenement deja v1", "../serveur/importer_journal.py",
     '    if mode in MODES_V1 and "nonce" in v0:',
     '    if False:'),
    ("la preparation ignore un fichier source ajoute", "migre_banque.py",
     '        rapport["source_inchangee"] = empreintes_source(racine) == rapport["empreintes_source"]',
     '        rapport["source_inchangee"] = all(empreintes_source(racine).get(nom) == sha for nom, sha in rapport["empreintes_source"].items())'),
    ("la preparation invente un auteur absent de la banque", "migre_banque.py",
     '            carte["provenance"] = {\n                "genere_le":',
     '            carte["provenance"] = {\n                "auteur": "humain", "genere_le":'),
    ("la preparation peut ecrire dans sa propre source", "migre_banque.py",
    ...
    ("la synchronisation perd les lignes recues dans la meme seconde", "../serveur/academie_etat/journal.py",
     "AND recu_le >= ? ORDER BY", "AND recu_le > ? ORDER BY"),
    ("le journal accepte les modes sans leurs champs obligatoires", "../serveur/academie_etat/journal.py",
     '    for champ in OBLIGATOIRES_PAR_MODE[ligne["mode"]]:', "    for champ in ():"),
    ("le programme accepte un prérequis de niveau supérieur", "valide_programme.py",
     '            elif ids[pre].get("niveau", 0) > niv:', "            elif False:"),
    ("un ajout IFSI peut inventer son historique", "valide_ifsi.py",
     '            if legacy != {"niveau": None, "titre": None, "ue_ids": []}:',
     '            if False:'),
    ("un prérequis IFSI peut arriver après le chapitre", "valide_ifsi.py",
