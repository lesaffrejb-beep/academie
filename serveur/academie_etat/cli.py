"""`academie-etat` : l'outil de ligne de commande de JB sur le VPS.
    ...
    python3 -m academie_etat profil creer "JB"
    ...
    python3 -m academie_etat reinitialiser-comptes --confirmer 'SUPPRIMER LES COMPTES'
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sp.add_parser("profil"); p.add_argument("action", choices=("creer", "lister")); p.add_argument("titre", nargs="?")
    j = sp.add_parser("jeton"); j.add_argument("profil"); j.add_argument("--appareil", default="outil")
    sp.add_parser("purger")
    r = sp.add_parser("reinitialiser-comptes"); r.add_argument("--confirmer")
