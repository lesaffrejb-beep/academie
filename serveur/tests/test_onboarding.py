...
class Onboarding(unittest.TestCase):
    ...
    def test_compte_connexion_et_sel(self):
        ...
        self.assertEqual(requete(self.a,"GET","/profil",cookie=c)[2]["cursus"],None)
    ...
    def test_journaux_et_cursus_isoles(self):
        ...
        self.assertEqual(requete(self.a,"POST","/journal",{"lignes":[event]},cookie=c)[0],200)
        self.assertEqual(requete(self.a,"GET","/profil",cookie=c)[2]["cursus"],"ifsi")
        self.assertEqual(requete(self.a,"POST","/journal",{"lignes":[]},cookie=d)[2]["manquantes"],[])
        self.assertEqual(requete(self.a,"POST","/journal",{"lignes":[ligne(2,mode="cursus",cursus="copro")]},cookie=c)[0],422)
        self.assertEqual(requete(self.a,"POST","/journal",{"lignes":[ligne(3,mode="cursus",cursus="inconnu")]},cookie=d)[0],422)
    def test_cookie_change_et_annuaire_prive(self):
        ...
        s,_,b=requete(self.a,"GET","/eleves",cookie=c)
        self.assertEqual(s,200);self.assertEqual(len(b["eleves"]),3)
        for e in b["eleves"]: self.assertEqual(set(e),{"id","pseudo","cursus"})
        self.assertEqual(requete(self.a,"PATCH","/profil",{"visibilite":False},cookie=d)[0],200)
        self.assertNotIn(q["id"],[e["id"] for e in requete(self.a,"GET","/eleves",cookie=c)[2]["eleves"]])
    def test_demandes(self):
        p,c=self.compte()
        self.assertEqual(requete(self.a,"POST","/demandes-cursus",{"texte":" "},cookie=c)[0],422)
        self.assertEqual(requete(self.a,"POST","/demandes-cursus",{"texte":"Apprendre un autre métier"},cookie=c)[0],201)
