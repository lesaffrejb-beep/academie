...
0. Qui tu es. Écris en une ligne l'outil et le modèle que tu es (menu du modèle, ou commande /model). Si tu n'en es pas sûr, tu es « inconnu ». Tu liras MODELES.md dès que le dépôt sera cloné et tu respecteras ta classe : petit, moyen ou grand. En cas de doute, petit.
...
4. Les documents, un par un, pas à pas. Pour chaque document, dans le dépôt produit :
   ...
   `python3 app/usine/usine.py declarer <empreinte> --outil <ton outil> --modele <ton modèle> --classe <ta classe>` ;
...
7. Ce que tu ne fais pas. Tu n'écris ni chapitre ni carte (c'est un chantier séparé, avec un grand modèle et une relecture). Tu n'appelles aucune API, tu n'installes aucun service. Tu ne scrapes aucun site de cours. Tu n'écris aucun nom de personne, aucune donnée de client ou de patient. Tu ne pousses rien vers un serveur. Tu n'écris jamais « fait », « relu » ou « validé » sans le verdict du script. Tu ne modifies pas `.etat.json` à la main : ça ne sert à rien, le script rejoue les contrôles.
