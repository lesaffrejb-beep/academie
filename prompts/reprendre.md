...
0. Qui tu es. Écris en une ligne l'outil et le modèle que tu es ; si tu n'en es pas sûr, « inconnu ». Lis MODELES.md, AGENTS.md et decisions/0027. Si l'outil ou le modèle a changé depuis la dernière déclaration, redéclare-toi (étape 2) : la taille des unités repart de celle de ta classe.
...
2. La déclaration. `python3 app/usine/usine.py declarer <empreinte> --outil <ton outil> --modele <ton modèle> --classe <ta classe>` si tu n'es pas celui qui avait déclaré.
