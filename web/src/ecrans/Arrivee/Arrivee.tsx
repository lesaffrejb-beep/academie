...
function Arrivee({compte,surCompte}:{compte:Compte|null;surCompte:(c:Compte)=>void}) {
  ...
  const ancienProfil=compte?.compte_personnel===false;
  const personnel=Boolean(compte && !ancienProfil && compte.cursus);
  const [mail,setMail]=useState(""); const [mdp,setMdp]=useState(""); const [pseudo,setPseudo]=useState(compte?.titre_affiche ?? "");
  ...
  async function soumet(e:FormEvent) {
    ...
    const r = await (ancienProfil ? api.activeCompte(mail,mdp,pseudo) : connexion ? api.connexion(mail,mdp) : api.inscription(mail,mdp,pseudo));
  ...
  return <div className="arrivee-page"><header className="arrivee-entete"><a href="#/">Académie</a><ChoixTheme /></header><main className="arrivee">
    <aside className="arrivee-intro"><BookOpen size={36} strokeWidth={1.2}/><p className="arrivee-repere">Ton métier, un peu plus loin.</p><h1>{personnel ? "Ton compte, ton parcours." : compte && !ancienProfil ? "Choisis où commencer." : "Un espace pour apprendre. Le tien."}</h1><p>Un cas pour réfléchir. Des sources pour comprendre. Des questions pour faire revenir ce qui compte.</p><ul><li><Check size={18}/> Tes réponses et ta progression restent dans ton espace.</li><li><Check size={18}/> Tu retrouves ton cursus en te reconnectant.</li><li><Check size={18}/> Tu choisis si les autres élèves te voient.</li></ul></aside>
    <section className="arrivee-formulaire" aria-label={personnel || ancienProfil || !compte ? "Compte" : "Choix du cursus"}>
      {erreur && <p role="alert" className="arrivee-erreur">{erreur}</p>}
      {personnel && compte ? <><h2>{compte.titre_affiche}</h2><p>Ton compte est connecté. Tes réponses validées se synchronisent dans ton espace personnel.</p><div className="compte-actions"><button className="action-etude" onClick={()=>va("/")}>Continuer mon parcours<ArrowRight size={19}/></button><button onClick={()=>va("/arbre")}>Voir mon arbre</button><button onClick={()=>va("/eleves")}>Mes sauvegardes et les élèves</button><button disabled={occupe} onClick={()=>void changeCompte()}>Utiliser un autre compte</button></div><Reprise pseudo={compte.titre_affiche}/></> : !compte || ancienProfil ? <>{ancienProfil && <p>Tu utilises un ancien espace. Crée tes identifiants pour conserver ses réponses et le retrouver sur tes appareils.</p>}<div className="arrivee-modes"><button aria-pressed={!connexion} onClick={() => {setConnexion(false);setErreur("");}}>Créer mon compte</button><button aria-pressed={connexion} onClick={() => {if(ancienProfil) {void changeCompte();return;} setConnexion(true);setErreur("");}}>{ancienProfil ? "Utiliser un autre compte" : "Me connecter"}</button></div><h2>{ancienProfil ? "Garder mon travail sur mon compte" : connexion ? "Retrouver ton espace" : "Bienvenue à l’Académie"}</h2><form onSubmit={e => void soumet(e)}>
      ...
      {compte && !ancienProfil && !personnel && <button disabled={occupe} onClick={()=>void changeCompte()}>Utiliser un autre compte</button>}
